...
def portfolio_home(request):
	projects = Project.objects.filter(status='p').select_related().order_by("-pk")
	return {'projects':projects}

@render_to('portfolio/portfolio_single.html')
def portfolio_single(request, project_slug):
	project = get_object_or_404(Project.objects.filter(status='p').select_related().order_by("-pk"), slug=project_slug)
