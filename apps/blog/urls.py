...
urlpatterns = patterns('blog.views',
    ...
    url(r'^article/(?P<post_slug>[\w-]+)/$', cache_page(article_single, 60*15)),
    url(r'^short-post/(?P<post_slug>[\w-]+)/$', cache_page(shortpost_single, 60*15)),
    url(r'^case-study/(?P<post_slug>[\w-]+)/$', cache_page(casestudy_single,60*15)),
    url(r'^quote/(?P<post_slug>[\w-]+)/$', cache_page(quote_single,60*15)),
